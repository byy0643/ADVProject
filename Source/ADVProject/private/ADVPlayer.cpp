...
void AADVPlayer::InputAim() {
	...
	if (_crosshairUI != nullptr && IsValid(_crosshairUI) && !_crosshairUI->IsUnreachable()) {
...
void AADVPlayer::InputInteract() {
	...
	if (bHit) {
		...
		IIInteractItems* intercativeActor = Cast<IIInteractItems>(hitComp);
		if (intercativeActor != nullptr) {
			UE_LOG(LogTemp, Log, TEXT("%s"), *hitComp->GetName());
			intercativeActor->Reaction();
